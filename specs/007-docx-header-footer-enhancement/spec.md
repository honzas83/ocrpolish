...
- Q: Formatting of the "PDF Page N" Footer → A: Combine "PDF Page N" and any right-aligned filtered metadata in the same footer area. (Superseded by: PDF Page N right-aligned on same line as centered -X-).
- Q: Handling of Multiline Filtered Metadata → A: Join with a specific separator (e.g., " | " or "; ") on a single line. (Final decision: Join with space " " on a single centered line).
...
**Independent Test**: Place a filtered line (e.g., "CONFIDENTIAL") before a `-1-` marker at the top of a page. Verify it moves to the Header, centered on its own line.
...
3. **Given** multiple filtered lines near a marker, **When** exported, **Then** they are concatenated with spaces and centered on a single line.
...
- **FR-003**: The system MUST ensure the "PDF Page N" footer is always present and right-aligned. It MUST appear on the same line as the centered `-X-` marker.
...
- **FR-006**: **Header Layout**:
    - Line 1: All filtered lines near the *first* (top-most) `-X-` marker, concatenated with spaces and centered.
    - Line 2: The *first* (top-most) `-X-` marker, centered.
- **FR-007**: **Footer Layout**:
    - Line 1: The *last* (bottom-most) `-X-` marker (centered) and "PDF Page N" (right-aligned).
    - Line 2: All filtered lines near the *last* (bottom-most) `-X-` marker, concatenated with spaces and centered.
- **FR-008**: **Margins**: The system MUST use reduced top/bottom margins of 0.3 inches to accommodate dense content.
...
- **SC-001**: 100% of pages contain the mandatory "PDF Page N" footer in the correct right-aligned position.
...
- **SC-004**: Metadata lines are correctly centered and separated from page numbers by zero-spacing paragraphs.
...
- The logic for moving metadata to DOCX headers/footers is strictly layout-specific and DOES NOT affect the content of `.filtered.md` sidecar files.
